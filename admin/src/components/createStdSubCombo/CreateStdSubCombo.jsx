...
function CreateStdSubCombo() {
  ...
  useEffect(() => {
    ...
    const getAllStudentSubject = async () => {
      try {
        const response = await secureAxios.get("/student", {
  ...
  const handleClick = async (e) => {
    ...
    if (studentId === "" || subjectId === "")
      return toast("pls fill in the space");
    const reqstData = { studentId, subjectId };
    AxiosPostPatch(
      "/combo",
      secureAxios.post,
      "Student and Subject Assigned Successfully",
      reqstData
    );
  };
  return (
              ...
              >
                ...
                {allstudent.map((student) => (
                  <option key={student._id} value={student._id}>
                    {student.fullname} ({student?.studentClassId?.className})
                  </option>
                ))}
              </select>
            </div>
